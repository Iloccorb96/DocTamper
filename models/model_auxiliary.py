...
#         else:
    ...
    def forward(self, x):
        ...
        if not self.multi:
            ...
            return self.norm1(self.stages[1](self.downsample_layers[1](x)))
...
#                 nn.BatchNorm2d(dim_in, **kwargs),
#                 nn.ReLU(),
#                 nn.Conv2d(dim_in, proj_dim, kernel_size=1)
#             )

#     def forward(self, x):
#         return F.normalize(self.proj(x), p=2, dim=1)#特征上进行L2范数归一化
...
#         self.proj = ProjectionHead(dim_in=in_channels, proj_dim=self.proj_dim)
        self.initialize()
        
        self.register_buffer("segment_queue_x", torch.randn(10000, 64))
        self.segment_queue_x = nn.functional.normalize(self.segment_queue_x, p=2, dim=1)
        self.register_buffer("segment_queue_y", torch.full((10000,), -1))
        self.register_buffer("segment_queue_ptr", torch.zeros(1, dtype=torch.long))
        
        self.register_buffer("pixel_queue_x", torch.randn(10000, 64))
        self.pixel_queue_x = nn.functional.normalize(self.pixel_queue_x, p=2, dim=1)
        self.register_buffer("pixel_queue_y", torch.full((10000,), -1))
...
#         print(transnext_rst[0].shape,transnext_rst[1].shape)
        ...
        emb = decoder_output
        return self.segmentation_head(decoder_output),emb
