...
class OverlapPatchEmbed(nn.Module):
    ...
    原始模型需要，魔改以后的，输入为embedding后的，不需要再embedding
    """
...
class TransNeXt(nn.Module):
    '''
    "img size" 参数主要用于生成相对空间坐标，这些坐标用于计算连续的相对位置偏置。
    由于这种 TransNeXt 实现可以接受多尺度输入，建议将 "img size" 参数设置为接近推理图像分辨率的值。
    不建议将 "img size" 参数设置为超过 800x800 的值。

    "pretrain size" 指的是在初始预训练阶段使用的 "img size"，
    它用于缩放相对空间坐标，以便通过 MLP 更好地进行外推。
    对于在 224x224 分辨率下训练的 ImageNet-1K 的模型，
    以及基于这些预训练权重进行微调的下游任务模型，"pretrain size" 参数应设置为 224x224。
    '''
        ...
        for i in range(1,num_stages):
            ...
            if i in [2,3]:
                patch_embed = OverlapPatchEmbed(patch_size=patch_size * 2 - 1 if i == 0 else 3,
                                  ...
                                  embed_dim=embed_dims[i])#out_chans
...
#             self.init_weights(pretrained)
    ...
    def forward_features(self, x):
        #修改前向传播函数，使得其架构类似于dtd中的swin transformer
        x, (H, W) = x
        B = x.shape[0]
        outs = []

        for i in range(1,self.num_stages):
            # 使用 getattr 可以避免使用大量的 if-elif-else 语句或多个单独的变量来存储属性引用，使代码更加简洁和灵活。
            ...
            if  i in [1,2]:
                patch_embed = getattr(self,f"patch_embed{i + 2}")
                x, H, W = patch_embed(x)
            if i in [1,3]:
                outs.append(x)
        return outs
