...
import collections
#使用 namedtuple 定义了 EfficientNet 中使用的块参数和全局参数。
...
class MBConvBlock(nn.Module):
    # 这是一个 Mobile Inverted Residual Bottleneck Block，它是 EfficientNet 模型的核心组件。这个块包括扩展卷积、深度卷积、SE模块和投影卷积。
    ...
    def set_swish(self, memory_efficient=True):
        '''
       Swish 激活函数是一种在深度学习中效果较好的激活函数，相比于 ReLU，它具有平滑和非线性的特性。Swish 可以由以下公式定义:Swish(x)=x⋅σ(x),其中σ(x) 是 Sigmoid 函数。
       set_swish 方法的作用是根据 memory_efficient 参数，选择使用 MemoryEfficientSwish 或标准的 Swish 作为激活函数
        '''
        self._swish = MemoryEfficientSwish() if memory_efficient else Swish()

class AddCoords(nn.Module):
    # 这是一个坐标添加模块，用于在特征图上添加坐标信息，增强卷积神经网络的位置感知能力。
